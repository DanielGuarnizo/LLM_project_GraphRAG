...
class SensemakingEvaluator:
    def __init__(
        ...
        self.pipeline = rag_pipeline
        self.llm = ChatOpenAI(model_name=eval_model)
        self.criteria = criteria or ["comprehensiveness", "diversity", "empowerment", "directness"]
    ...
    def judge_one_criterion(self, query: str, answer_1: str, answer_2: str, criterion: str, description: str) -> Dict:
        system_prompt = f"""
        You are a helpful assistant responsible for comparing two answers to a question based on one criterion.

        ---Criterion---
        {criterion.capitalize()}: {description}

        ---Instructions---
        Given the question and two answers below, choose which is better for the criterion.
        ...
        try:
            ...
            return {
                "winner": "error",
                "reasoning": f"⚠️ LLM evaluation error on {criterion}: {e}"
            }
    ...
    def evaluate(self):
        ...
        with open(self.output_path, "w") as out_file:
            out_file.write("# 🧠 Global Sensemaking Evaluation Report (Markdown)\n\n")
            ...
            for entry in data:
                ...
                for question in entry.get("questions", []):
                    ...
                    out_file.write(f"**Persona:** {persona}  \n")
                    out_file.write(f"**Task:** {task}\n\n")
                    out_file.write(f"### 🗣️ Question\n{question}\n\n")
                    out_file.write(f"### 🧪 Answer 1 (Vector RAG)\n{answers['1']}\n\n")
                    out_file.write(f"### 🧪 Answer 2 (Graph RAG)\n{answers['2']}\n\n")

                    for criterion in self.criteria:
                        desc = CRITERIA.get(criterion, "No description available.")
                        judgment = self.judge_one_criterion(
                            query=question,
                            answer_1=answers["1"],
                            answer_2=answers["2"],
                            criterion=criterion,
                        ...
                        out_file.write(f"### 🧠 Evaluation — {criterion.capitalize()}\n")
                        out_file.write("```json\n")
                        out_file.write(json.dumps(judgment, indent=2))
                        out_file.write("\n```\n\n")
